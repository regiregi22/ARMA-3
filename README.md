...
7- Start ARMA 3 Launcher, select PARAMETERS and go to ALL PARAMETERS tab. Set "Platform" to "64-bit", tick "Show static background in menu", tick "Skip logos at startup", tick "Enable Large-page Support", tick "No Logs", set "World" to "empty", tick "No Pause", tick "No Pause Audio".  
...
19- Set Windows Energy plan to "Ultimate Performance": https://www.ghacks.net/2024/12/04/how-to-enable-the-ultimate-performance-plan-in-windows-11/

20- Restart the computer.  

21- Press Start button, search "Disk Cleanup", select C:, mark "DirectX Shader Cache" and click OK.

22- Delete Nvidia Shader Cache folders "DXCache" and "GLCache": https://nvidia.custhelp.com/app/answers/detail/a_id/5735/~/deleting-nvidia-shader-cache-files

23- Run Arma 3, go to Options, Video Options and click the button "AUTODETECT". Go to tab Display, set Display mode to "Fullscreen", set your resolution, set your aspect ratio, set "VSYNC" to Disabled, set "Anisotropic filtering" to 16x. This is your base configuration, if you want now you can fine tune the settings on the "General" tab and the "AA & PP" tab to your liking. You can see here if a setting is more CPU or GPU dependant:
https://community.bistudio.com/wiki/Arma_3:_Performance_Optimisation

24- If running on a high end modern computer (as an Intel 10900K, Nvidia RTX 3080 and 16Gb RAM) you can just max out everything as shown in those three screenshots a, b and c, and still get +100fps on a 4K screen: https://github.com/regiregi22/ARMA-3/tree/main/img

25- This is the gold standard benchmark for testing Arma performance settings, if needed:
...
-D:\SteamLibrary\steamapps\common\Arma 3\MyArmaServer\Users\myserver\myserver.Arma3Profile (Difficulty settings)  
-D:\SteamLibrary\steamapps\common\Arma 3\MyArmaServer\Users\myserver\myserver.vars.Arma3Profile (Some binarised content which you cannot edit)  
-D:\SteamLibrary\steamapps\common\Arma 3\MyArmaServer\basic.cfg (Network Bandwidth settings)  
